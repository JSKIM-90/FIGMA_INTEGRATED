...
문제 1: 500개 드래그 앤 드롭
문제 2: 500개 위치/크기 조정
문제 3: 500개 자산 ID 매핑  ← 이것만 문제가 아님
```

**자산 매핑은 배치 작업의 여러 속성 중 하나일 뿐, 별도의 특수한 문제가 아니다.**

"자산 매핑 문제"라고 명명하는 순간 **문제를 과대 정의**하게 된다.
- 배치가 많으면 → 모든 게 문제 (자산 매핑 포함)
- 배치가 적으면 → 자산 매핑도 문제 아님

따라서 "자산 매핑을 위한 별도 솔루션"을 고민할 필요가 없다.
이는 **웹 빌더의 근본적 성격(수동 배치 도구)**에서 파생되는 자연스러운 한계이다.
